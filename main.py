...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    num_nodes = [10, 20, 50, 80, 100, 120]
    loaders =[ ]
    valid_loaders = []

    for num_node in range(len(num_nodes)):
    ...
    for loader_idx in range(len(loaders)):

        train_loader = loaders[loader_idx]
        ...
        for epoch in range(epochs):
            ...
            mean_valid = validate(valid_loaders[loader_idx], actor, reward_fn, render_fn,
...
if __name__ == '__main__':
    ...
    for num_nodes_idx in range(len(num_nodess)):
        ...
        experiment_name = f'trained_on_many_nodes_CVRP_nodesSize{train_size}_bs{bs}_actLr{actor_lr}_criLr{critic_lr}'

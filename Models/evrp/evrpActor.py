...
class NetworkEVRP(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size,
                 update_fn=None, mask_fn=None,
                 embedding='conv',
                 num_layers=1, dropout=0.):
        super(NetworkEVRP, self).__init__()
        ...
        self.static_encoder = Encoder(static_size, hidden_size, embedding)
        self.dynamic_encoder = Encoder(dynamic_size, hidden_size, embedding)
        self.decoder = Encoder(static_size, hidden_size, embedding)
    ...
    def forward(self, static, dynamic,distances, decoder_input=None, last_hh=None):
        """
        Parameters
        ----------
        ...
        capacity = 60
        velocity = 60
        cons_rate = 0.1  # fuel consumption rate (gallon/mile)
        t_limit = 40
        num_afs = 3
        ...
        for step in range(max_steps):
            #print(f'Step {step}')
            #numberOfUnsatisfied = len(((dynamic[:, 2, :] == 0)==False).nonzero())
            ...
            probs = F.softmax(mask + probs, dim=1)
            ...
            if self.update_fn is not None:
                dynamic = self.update_fn(old_idx, ptr.data.unsqueeze(1),
                                         mask,
                                         dynamic,
                                         distances,
                                         dis_by_afs, capacity,
                ...
                assert dynamic.size(1) == 3 #number of dynamic features
                ...
                is_done = dynamic[:,2,:].sum(1).eq(0).float() # is the demand = 0?
                logp = logp * (1. - is_done)

                old_idx = ptr.data.unsqueeze(1)
        ...
        return tour_idx, tour_logp, step
...
from explainableAI import explain_decision
    ...
    def forward(self, static, dynamic, decoder_input=None, last_hh=None):
        ...
        for i in range(max_steps):
            ...
            if self.training:
                ...
                ################ EXPLAINABLE AI PART ######################
            # if i > 2 or i >= 3 and torch.cat(tour_idx, 1)[0].size(0) == 3:
            #         if m is not None:
            #             chosen_index = ptr.data.detach()
            #             # exei kanei torch.cat(tours,1)[0] tour kai dialegei to
            #             # chosen_indexes[0]
            #             explain_decision(i, static[0], dynamic[0], distribution=m,
            #                          current_tour=torch.cat(tour_idx, 1)[0],
            #                          chosen_city=chosen_index[0])

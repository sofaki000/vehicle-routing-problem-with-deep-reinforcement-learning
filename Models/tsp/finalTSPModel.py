...
from Models.critc import StateCriticTSP
from utils.plots import plot_two_models_metrics
...
class TrainModel:
    def __init__(self, model, train_dataset, val_dataset, batch_size=128, threshold=None, max_grad_norm=2., modelName="model.pt"):
        self.model = model
    ...
    def train_and_validate(self, n_epochs):
        ...
        return self.train_loss, self.advantage, self.train_tour, self.val_tour
    ...
    def plotTrainLossAndAdvantage(self, epoch):
        ...
        plt.grid()
        plt.subplot(132)
        plt.title('Advantage: epoch %s reward %s' % (epoch, self.advantage[-1] if len(self.advantage) else 'collecting'))
    ...
    def plotTrainAndValidationTourLength(self, epoch):
        ...
        plt.grid()
        plt.subplot(132)
        plt.title('val tour length: epoch %s,tour length %s' % (epoch, self.val_tour[-1] if len(self.val_tour) else 'collecting'))
...
class TrainModelActorCritic:
    def __init__(self, model,critic, train_dataset, val_dataset, batch_size=128, threshold=None, max_grad_norm=2., modelName="model.pt"):
        self.model = model
        self.critic = critic
        self.train_dataset = train_dataset
        self.val_dataset   = val_dataset
        ...
        self.actor_optim   = optim.Adam(model.actor.parameters(), lr=1e-4)
        self.critic_optim =  optim.Adam(critic.parameters(), lr=1e-1)
    ...
    def plotTrainLossAndAdvantage(self, epoch):
        ...
        plt.ylabel("Training advantage")
        plt.xlabel("Epoch")
        plt.savefig(f"CRITICtrainLossAndAdvantage{self.modelName}.png")
    ...
    def plotTrainAndValidationTourLength(self, epoch):
        ...
        plt.plot(self.val_tour)
        plt.grid()
        plt.savefig(f"CRITICtourLength{self.modelName}.png")
...
if __name__ =="__main__":
    ...
    train_size = 15_000  # 100000
    val_size = 1000
    num_nodes = 20
    epochs = 20
    train_20_dataset = TSPDataset(num_nodes, train_size)
    val_20_dataset = TSPDataset(num_nodes, val_size)
    ...
    train_loss,advantage, train_tour, val_tour = tsp_20_train.train_and_validate(epochs)

    ## training with actor critic

    actor = CombinatorialRL(
        embedding_size,
        hidden_size,
        num_nodes,
        n_glimpses,
    ...
    critic = StateCriticTSP(2, hidden_size, "conv")

    tsp_20_train_actor_critic = TrainModelActorCritic(tsp_20_model_dot_attention,
                                          critic,
                                          train_20_dataset,
                                          val_20_dataset,
                                          modelName="tsp_20_dot_attention.pt",
                                          threshold=0.5)
    critic_train_loss,critic_advantage, critic_train_tour, critic_val_tour = tsp_20_train_actor_critic.train_and_validate(epochs)

    experiment_name="actorCritic_vs_criticavgmean"


    model_1_metrics = [ train_loss,advantage, train_tour, val_tour]

    model_2_metrics = [critic_train_loss,critic_advantage, critic_train_tour, critic_val_tour]
    labels = ["Moving average baseline", "Actor-critic"]
    metric_labels =["Train loss", "Advantage", "Train tour length", "Validation tour length"]
    plot_two_models_metrics(model_1_metrics, model_2_metrics, labels, metric_labels, experiment_name)
